...
RASTERS_DIR = os.path.join(BASE_DIR, "outputs", "rasters")
...
OUTPUT_RASTER = os.path.join(RASTERS_DIR, "classified_hunga_tonga.tif")
OUTPUT_AREA_CSV = os.path.join(METRICS_DIR, "area_by_class.csv")
OUTPUT_MAPPING_CSV = os.path.join(METRICS_DIR, "class_mapping.csv")
...
try:
    ...
    print("Clasificando raster completo por bloques...")
    ...
    with rasterio.open(OUTPUT_RASTER, "w", **output_profile) as dst:
        ...
        for row_off in range(0, ref_height, block_size):
            for col_off in range(0, ref_width, block_size):
                ...
                # Se excluyen píxeles con NaN, inf o valores <= 0 en alguna banda.
                valid_mask = (
                ...
                if valid_mask.sum() > 0:
                    ...
                    pred_labels = model.predict(valid_pixels)

                    # El modelo predice 0,1,2,3,4.
                    # En el raster guardamos 1,2,3,4,5.
